...
./mvnw clean
echo "\n|----prebuild----|" >> docs/speedtest
ts=$(date +%s%N) ; ./mvnw package verify ; tt=$((($(date +%s%N) - $ts)/1000000)) ; echo "prebuild took: $tt milliseconds" >> ../docs/speedtest
cd ..
ts=$(date +%s%N) ; docker build -f Dockerfile.demo.prebuild -t demo:dockerfile.demo.prebuild . ; tt=$((($(date +%s%N) - $ts)/1000000)) ; echo "dockerfile.demo.prebuild took: $tt milliseconds" >> docs/speedtest
docker rmi demo:dockerfile.demo.prebuild
ts=$(date +%s%N) ; packer build packer.demo.prebuild.json ; tt=$((($(date +%s%N) - $ts)/1000000)) ; echo "packer.demo.prebuild took: $tt milliseconds" >> docs/speedtest
